...
result<-train_svmlinear$results #总的指标
foldresult<-train_svmlinear$resample #每一次每一折的指标
predrepeatedcv<-train_svmlinear$pred #这是所有次的交叉验证所有折的预测结果
fit_svm_mod<-train_svmlinear$finalModel #这是得到的最终模型

# 需要整理一下预测的结果
predrepcv<-arrange(predrepeatedcv,rowIndex) #按rowIndex排序
predrepcv2<-predrepcv[,3:5] #取需要的列
...
ggplot(predresult,
  ...
  labs(fill=NULL,colour=NULL,
       ...
       title = "SVM Experiment Compared with Predict")+
  theme_classic()+
  theme_bw()+
