...
result<-train_rf$results #总的指标
foldresult<-train_rf$resample #每一次每一折的指标
predcv<<-train_rf$pred # 一次交叉验证的话预测结果是这个
predrepeatedcv<-train_rf$pred #这是所有次的交叉验证所有折的预测结果
fit_rf_mod<-train_rf$finalModel #这是得到的最终模型

# 需要整理一下预测的结果（这是重复多次交叉验证的结果）
predrepcv<-arrange(predrepeatedcv,rowIndex) #按rowIndex排序
predrepcv2<-predrepcv[,2:4] #取需要的列
...
# 一次交叉验证的结果整理
predcv<-arrange(predcv,rowIndex) #按rowIndex排序
predtrain<-data.frame(predcv$pred)
trainpred_data<-cbind(data_train,predtrain)

# 测试集预测结果
testpred<-predict(fit_rf_mod,
                  newdata = data_testx)
# 测试集预测误差指标
...
ggplot(predresult,
  ...
  labs(fill=NULL,colour=NULL,
       ...
       title = "randomForest Experiment Compared with Predict")+
  theme_classic()+
  theme_bw()+
...
# 原始的交叉验证预测值（一次）
write.csv(predcv,file = "predcv.csv")

# 变量重要性
rfimp<-fit_rf_mod$importance
varImpPlot(fit_rf_mod,n.var = min(20, nrow(fit_rf_mod$importance)),
           main = 'Top 20 - variable importance')
write.csv(rfimp,file = "rfimp.csv")
